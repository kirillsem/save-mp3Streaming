...
def Usage():
  print
  print "Usage: save-mp3Streaming.py [options] m3u_file"
  print
  ...
  print "                (highly recommended for use with mp3.com) "
  print "       -i       use ICY title data from a shoutcast stream "
  print "                (this option is much improved) "
  print "       -l       log output to save-mp3Streaming.log file "
  ...
  print "xterm -e save-mp3Streaming.py -v -n 3 %s"
  print
  sys.exit()
...
class Logger:
  ...
  # a race condition if there are tons of save-mp3Streaming scripts
  # starting at the same time and should really use locking
  # but that would be overkill for this app
  #
  def __init__(self, logname = "save-mp3Streaming.log", verbose = 0, logging = 0):
    self.logname = logname
    self.verbose = verbose
    self.logging = logging
    self.prefix = "save-mp3Streaming: "
    if self.logging:
      self.logfp = open(self.logname, "w")
      self.logfp.write("save-mp3Streaming v" + Version + "\n")
...
if __name__=="__main__":
  ...
  if sys.platform == "win32":
    print "save-mp3Streaming v" + Version + " win32"
  else:
    print "save-mp3Streaming v" + Version + " linux"
  ...
  logname = fnfactory.makeindir(opt_topdir, "save-mp3Streaming.log")
  ...
  for url in urls:
    ...
    try:
      ...
      while 1:
        ...
        kilobytes = bytesread / 1000
        print "Data read: " + `kilobytes` + "K     CTRL-C to stop\r",
        ...
        if opt_icydata == 0:
          ...
          if bytesread <= icycount * icy.getmetaint():
        ...
        if opt_size > 0 and bytesread > opt_size * 1000000:
          urlfp.close() 
          tmpfp.close()
          if opt_icydata == 2:
            newmp3name = fnfactory.makeindir(opt_topdir, icyfilename)
            os.rename(tmpfilename, newmp3name)
            logline = "size limit after " + `bytesread` + " bytes saved to " + newmp3name
          else: 
      ...
      if opt_icydata == 2:
        newmp3name = fnfactory.makeindir(opt_topdir, icyfilename)
        os.rename(tmpfilename, newmp3name)
        logline = "keyboard interrupt after " + `bytesread` + " bytes saved to " + newmp3name
      else: 
        newmp3name = fnfactory.makeindir(opt_topdir, opt_unfinished)
        os.rename(tmpfilename, newmp3name)
        logline = "keyboard interrupt after " + `bytesread` + " bytes saved to " + newmp3name
